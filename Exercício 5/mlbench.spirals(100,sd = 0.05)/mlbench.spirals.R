...
# Gerar o conjunto de dados Spirals com 100 pontos e desvio padrão de 0.05
...
plot(xin_treino[yin_treino == -1, 1], xin_treino[yin_treino == -1, 2], col = "red", pch = 19,
     ...
     main = "Estrutura dos Dados de Treino (Spirals)", xlab = "X1", ylab = "X2")
...
plot(xin_validacao[yin_validacao == -1, 1], xin_validacao[yin_validacao == -1, 2], col = "red", pch = 19,
     xlim = range(xin), ylim = range(xin),
     main = "Estrutura dos Dados de Validação (Spirals)", xlab = "X1", ylab = "X2")
...
p <- 500  # Número de neurônios na camada oculta
...
# Usar paste() para incluir o valor de p no título
main_title <- paste("Superfície de Separação (Spirals) p =", p)
